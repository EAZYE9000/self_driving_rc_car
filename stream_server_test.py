...
label_array_2 = np.zeros((1, 4), 'float')

server_socket = socket.socket()
server_socket.bind(('192.168.1.85', 8000))
...
while True:
    ...
    for event in pygame.event.get():
        if event.type == KEYDOWN:
            ...
            if key_input[pygame.K_UP] and key_input[pygame.K_RIGHT]:
                ...
                cv2.imwrite('dataset_images/image{:>05}.jpg'.format(counter), image)
                label_array_2 = np.vstack((label_array_2, k[1]))
                ...
                cv2.imwrite('dataset_images/image{:>05}.jpg'.format(counter), image)
                label_array_2 = np.vstack((label_array_2, k[0]))
                ...
                cv2.imwrite('dataset_images/image{:>05}.jpg'.format(counter), image)
                label_array_2 = np.vstack((label_array_2, k[2]))
                ...
                cv2.imwrite('dataset_images/image{:>05}.jpg'.format(counter), image)
                label_array_2 = np.vstack((label_array_2, k[3]))
    ...
    train_labels_2 = label_array_2[1:, :]

    np.savez('rc_car_dataset.npz', train=train, train_labels=train_labels, train_labels_2=train_labels_2)
    e2 = cv2.getTickCount()
